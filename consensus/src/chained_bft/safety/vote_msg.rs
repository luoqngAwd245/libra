...
// 限内部使用。 包含VoteMsgSerializer中有助于计算其哈希值的所有字段。
...
/// VoteMsg是由选民最终发送以响应接收提案的结构。
/// VoteMsg带有一个块的`LedgerInfo`，如果这个投票收集了QuorumCertificate（请参阅`LedgerInfo`注释中的详细解释），
/// 它将被提交。
